            ...
            conexion.query(sql, [partida], function (err, resultado) {
                if (!err) {
                    ...
                    sql = "select poner.Fila, poner.Columna, poner.ID_Carta, cartas.URL, cartas.Tipo, poner.Descubierta " +
                            "from poner, cartas " +
                            "where poner.ID_Partida = ? and poner.ID_Carta = cartas.ID;";
                    conexion.query(sql, [ID_partida], function (err, resultado) {
                        if (!err) {
            ...
            conexion.query(sql, [partida], function (err, resultado) {
                if (!err) {
                    ...
                    sql = "select usuario.Nombre_Usuario " +
                            "from poner, usuario " +
                            "where poner.ID_Usuario = usuario.ID and poner.ID_Carta = ?" +
                            "and poner.ID_Partida = ? " +
                            "and poner.Fila = ? and poner.Columna = ?;";

                    conexion.query(sql, [ID, ID_partida, fila, columna], function (err, resultado) {
                        if (!err) {
