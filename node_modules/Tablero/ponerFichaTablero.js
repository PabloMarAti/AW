...
function PonerFichaTablero(usuario, usuarioAfectado, partida, carta, fila, columna, callback) {
    ...
    conexion.connect(function (err) {
        if (!err) {
            var sql = "SELECT  Tipo FROM `cartas` WHERE cartas.ID = " + carta;

            conexion.query(sql, function (err, Tipocarta) {
                if (!err) {
                    var tipo = Tipocarta[0].Tipo;
                    sql = "select ID from partidas where partidas.Nombre = '" + partida + "';";
    ...
    conexion.connect(function (err) {
        if (!err) {
            var sql = "Update juega " +
                    "set Pico = '" + pico + "' " +
                    "where ID_Partida = '" + ID_partida + "' and ID_Usuario = '" + usuarioAfectado + "';";
            conexion.query(sql, function (err, resultado) {
                if (!err) {
                    callback(null, true);
                } else {
...
function ponerFichaLupa(ID_partida, tablero, fila, columna, aux, callback) {
    if (callback === undefined) {
        callback = function () {};
    }
    ...
    if (tablero[fila][columna] !== undefined && tablero[fila][columna].ID_Carta > 4) {
        conexion.connect(function (err) {
            if (!err) {
                var sql = "delete " +
                        "from poner " +
                        "where ID_Partida = '" + ID_partida + "' and Fila = '" + fila + "' and Columna = '" + columna + "';";
                conexion.query(sql, function (err, resultado) {
                    if (!err) {
                        callback(null, true);
                    } else {
...
function ponerFichaNormal(ID_partida, ID_usuario, tablero, aux, fila, columna) {
                    ...
                    comprobarValidezFicha(tablero, fila, columna, function (valida, CasillasDescubrir) {
                        if (LlegaAInicio && valida) {
                            ...
                            sql = "INSERT INTO `poner` " +
                                    ...
                                    "VALUES (null, '" + ID_usuario + "', '" + ID_partida + "', '" + aux.ID_Carta + "', '" + fila + "', '" + columna + "');";
                            conexion.query(sql, function (err, resultado) {
                                if (!err) {
