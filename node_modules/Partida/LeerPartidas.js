...
function LeerPartidasAbiertas(usuario, callback) {
    ...
    conexion.connect(function (err) {
        if (err) {
            ...
            var sql = "select ID from usuario where Nombre_Usuario = ?;";
            conexion.query(sql, [usuario], function (err, usuario) {
                if (!err) {
                    ...
                    sql = "select Nombre, Creador, Estado, Numero_Jugadores, GROUP_CONCAT(DISTINCT usuario.Nombre_Usuario SEPARATOR ', ') as JugadoresUnidos " +
                            ...
                            "where partidas.Estado = 'Abierta' and partidas.ID = juega.ID_Partida AND juega.ID_Usuario = usuario.ID and juega.ID_Partida NOT IN (select juega.ID_Partida from juega where juega.ID_Usuario = ?) " +
                            "GROUP BY juega.ID_Partida;";
                    conexion.query(sql, [IDUsuario], function (err, filas) {
                        if (err) {
                            callback(err);
                        } else {
                            callback(null, filas);
    ...
    conexion.connect(function (err) {
        if (err) {
            ...
            var sql = "select Nombre, Fecha, Numero_Jugadores, Turno " +
                    "from partidas" +
                    " where partidas.Estado = 'Abierta' and partidas.Creador = ?;";
            ...
            conexion.query(sql, [usuario], function (err, usuario) {
                if (!err) {
                    ...
                    var sql = "select distinct Nombre, Creador, Turno, Fecha " +
                            "from partidas, juega " +
                            "where partidas.Estado = 'Activa' and juega.ID_Partida IN (select juega.ID_Partida from juega where juega.ID_Usuario = ?) ";
                    conexion.query(sql, [IDUsuario], function (err, filas) {
                        if (err) {
                            callback(err);
                        } else {
            ...
            conexion.query(sql, function (err, usuario) {
                if (!err) {
                    ...
                    var sql = "select distinct Nombre, Creador, Estado, Numero_Jugadores, Ganador " +
                            "from partidas, juega " +
                            "where partidas.Estado = 'Cerrada'  and juega.ID_Partida IN (select juega.ID_Partida from juega where juega.ID_Usuario = ?); ";
                    conexion.query(sql, [IDUsuario], function (err, filas) {
                        if (err) {
                            callback(err);
                        } else {
...
function LeerPartidaActual(nombre, callback) {
    ...
    conexion.connect(function (err) {
        if (err) {
            ...
            var sql = "select partidas.ID as game, usuario.ID as user, Turno, Creador, Turnos_restantes, Numero_Jugadores, usuario.Nombre_Usuario, usuario.Foto, juega.Rol, Ganador, Pico " +
                    "from partidas, juega, usuario " +
                    "where partidas.Nombre = ? and partidas.ID = juega.ID_Partida AND juega.ID_Usuario = usuario.ID;";
