...
function ComprobarGanadorSaboteador(tablero, tableroVisto, fila, columna, callback) {
    ...
    if (casilla.length === 1) {
            ...
            if (f - 1 >= 0) {
                if (tablero[f - 1][c] !== undefined){
                    if (!tableroVisto[f - 1][c]) {
                        ComprobarGanadorSaboteador(tablero, tableroVisto, f - 1, c, function (err, resultadoArriba, tab) {
                            if (!err) {
        ...
        if (casilla === "Inicio" || (casilla.indexOf("d") !== -1)) {
            if (f + 1 <= 6) {
                if (tablero[f + 1][c] !== undefined){
                    if (!tableroVisto[f + 1][c]) {
                        ComprobarGanadorSaboteador(tablero, tableroVisto, f + 1, c, function (err, resultadoAbajo, tab) {
                            if (!err) {
            ...
            if (c + 1 <= 6) {
                if (tablero[f][c + 1] !== undefined) {
                    if (!tableroVisto[f][c + 1]) {
                        ComprobarGanadorSaboteador(tablero, tableroVisto, f, c + 1, function (err, resultadoDerecha, tab) {
                            if (!err) {
        ...
        if (casilla === "Inicio" || casilla.indexOf("l") !== -1) {
            if (c - 1 >= 0) {
                if (tablero[f][c - 1] !== undefined) {
                    if (!tableroVisto[f][c - 1]) {
                        ComprobarGanadorSaboteador(tablero, tableroVisto, f, c - 1, function (err, resultadoIzquierda, tab) {
                            if (!err) {
...
function ComprobarGanadorMineros(partida, tablero, callback) {
    ...
    conexion.connect(function (err) {
        if (!err) {
            var sql = "select ID from partidas where partidas.Nombre = ?;";
            ...
            conexion.query(sql, [partida], function (err, resultado) {
                if (!err) {
                    ...
                    sql = "Select Fila, Columna " +
                            "from poner " +
                            "where ID_Partida = ? and ID_Carta = 2;";

                    conexion.query(sql, [ID_partida], function (err, oro) {
                        if (!err) {
